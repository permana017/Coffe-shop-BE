            ...
            if (err) {
              ...
              for (let index = 0; index < file.length; index++) {
                db.query(
                  ...
                  [uuidv4(), result.rows[0].id, tittle, file[index].filename]
                );
              }
              return resolve({
                tittle,
      ...
      return db.query(
        ...
        (err, result) => {
          if (err) {
            ...
            db.query(
              `UPDATE products SET tittle='${
                tittle || result.rows[0].tittle
              }', description='${
                description || result.rows[0].description
              }', price='${price || result.rows[0].price}',img='${
              ...
              (err, result) => {
                if (err) {
                  ...
                  db.query(
                    `SELECT image_id, filename FROM product_images WHERE product_id='${id}'`,
                    (errProductImages, productImages) => {
                      ...
                      for (
                        ...
                        db.query(
                          `UPDATE product_images SET filename=$1 WHERE image_id=$2`,
                          [
                            file[indexNew].filename,
                            productImages.rows[indexNew].image_id,
                          ],
                          (err, result) => {
                            if (err)
